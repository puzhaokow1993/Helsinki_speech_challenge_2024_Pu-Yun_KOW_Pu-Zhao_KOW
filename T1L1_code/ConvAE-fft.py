...
fft_magnitude_diff = output_fft_magnitude - input_fft_magnitude

# # Create the first figure for training loss
# plt.figure()
# plt.plot(input_fft_magnitude[0,:], label='input_fft', color='red')
# plt.plot(output_fft_magnitude[0,:], label='output_fft', color='blue')
...
history=model.fit(input_fft_magnitude, fft_magnitude_diff, epochs=100, batch_size=8)
...
batch_size=8
predicted_fft_magnitude = np.squeeze((model.predict(input_fft_magnitude,batch_size=batch_size))) 
predicted_fft = predicted_fft_magnitude * np.exp(1j * input_fft_phase)
# Perform inverse FFT to transform back to time domain
pred_data = np.fft.ifft(predicted_fft, axis=1).real  # Take the real part after IFFT


predicted_fft_diff = np.squeeze((model.predict(input_fft_magnitude,batch_size=batch_size))) 
predicted_output_fft_magnitude = input_fft_magnitude + predicted_fft_diff
predicted_output_fft = predicted_output_fft_magnitude * np.exp(1j * input_fft_phase)
