...
noisy_dir=r'D:\important\Hensinki_Speech_Challenge_2024\my_project\raw_dataset\Task_1_Level_4\Recorded'
noisy_audio_int=sampling(noisy_dir)
clean_dir=r'D:\important\Hensinki_Speech_Challenge_2024\my_project\raw_dataset\Task_1_Level_4\Clean'
...
specific_index=[16,516]

full_result = []
for i in range(len(specific_index)):
    print(i)
    
    original_text = real_words.iloc[specific_index[i]-1,1]
    transcribed_text = model.stt(noisy_audio_int[specific_index[i]-1])
...
df.to_csv(r"D:\important\Hensinki_Speech_Challenge_2024\my_project\result\%s\noise_result.csv"%(data_folder), index=False)
